...
def convert_version_string_to_int(string, number_bits):
    ...
    if len(numbers) > len(number_bits):
        error_message = (
            "Versions with more than {0} "
            "decimal places are not supported"
        )
        raise ValidationError(error_message.format(len(number_bits) - 1))
    ...
    for num, bits in reversed(list(zip(numbers, number_bits))):
        ...
        if num >= 1 << max_num:
            error_message = (
                "Number {0} cannot be stored "
                "with only {1} bits. Max is {2}"
            )
            raise ValueError(error_message.format(num, bits, max_num))
